...
class FriendsTableViewController: UITableViewController{
    ...
    override func viewDidLoad() {
        ...
        friendService.loadData { (friends) in
...
//            }
//        }
        let firstName = "\(friendsList[indexPath.row].fisrtName)"
        let lastName = "\(friendsList[indexPath.row].lastName)"
        let image = "\(friendsList[indexPath.row].photo)"
        if let url = URL(string: image) {
            cell.imageFriendView.image = UIImage(data: try! Data(contentsOf: url))
        }
